...
function firstPropertyOfType(properties, type) {
  return Object.entries(properties || {}).find(([, value]) => value && value.type === type)
}
...
function pageDescription(page) {
  ...
  const candidates = ['Resumo', 'Descrição', 'Description', 'Summary', 'Subtítulo']
  ...
  return rich ? richTextToPlain(rich[1].rich_text) : ''
}
  ...
  try {
    if (!process.env.NOTION_TOKEN || !process.env.NOTION_DATABASE_ID) {
      return res.status(200).json({
        source: 'fallback',
        warning: 'Configure NOTION_TOKEN e NOTION_DATABASE_ID no Vercel para ler o Notion.',
        items: []
      })
    }
